%% MAPS_example.m
% Sample closed loop scenario for MAPS
%%
...
%% display all figures? (if yes it will slow down the computation)
figDisp = 'yes';
...
wfsType = 'sh'; %'sh' or 'py'
modeType = 'zonal1'; %Modal basis correction - 'zonal2', 'zern', 'KL', 'DH'; 'zonal1' should be computed for a SHWFS while 'zonal2' is for a PWFS
...
switch wfsType
    case 'sh'
        ...
        nRes = nL*nPx;          % resolution on the pupil plane (no of pixels), make sure atleast nyquist sampled for r0 (so at least ~140 pix)
        quadCell = 'false';      %using a quadcell at fainter magnitudes helps the SHWFS (quadcell is 4x4 pixels per SHWFS subaperture)
...
%% ?? What doe these propogation operators do, what's the difference between 
%% .* and * does one set while the other propogates? OOMAO documentation 
%% not super helpful. Okay so when it's .* you reset the amplitude 
%% and phase first.
...
%% ?? What does a single + do? Updates the WFS to take a new measurements
+wfs;
%%
...
switch dmType
    ...
    case 'reg'
    %% ?? Don't quite understand what the influence function means.
    % Is is the location of the actuators?
    %% Valid actuator just allows you to remove actuators from action?
...
%% This poke is just for calibration.

%% The choice of mode is how you do your calibration. Do it in terms of 
% polynomials,  or just poke actuators
switch modeType
    ...
    case 'zonal1'
        ...
        % pokes the DM ad measures the phase changes
